...
function insertExp() {
	...
	return document.getElementById('display1').value = exp;
}	
...
function doMathOper(oper) {
	...
	switch (oper) {
		case 'root':
			if (Number(getResalt(exp))<0){return ifErrorId('display1');}
			var display2_text = "√(" + exp + ")";
			exp = Math.sqrt(Number(getResalt(exp)));
			break;
		case 'square':
			var display2_text = "(" + exp + ")^2";
			var exp = Number(getResalt(exp))**2;
			break;
		case 'div':
			var display2_text = "1/(" + exp + ")";
			var exp = 1/Number(getResalt(exp));
			break;
		case 'pers':
